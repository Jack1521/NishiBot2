...
const starts = async (dogexeon = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('NishiV2', {
		font: 'Standard',
	...
	try {
		...
		if (anu.action == 'add') {
			...
			try {
				...
				sendButImage(anu.jid, teks, `𝗝𝗮𝗰𝗸𝗩𝟮`, img,but = [{buttonId:`getdesc`, 
			...
			try {
				...
				sendButImage(anu.jid, teks, `𝗝𝗮𝗰𝗸𝗩𝟮`, img,but = [{buttonId: `Hello World!`, buttonText: {displayText: `GOODBYE !!`}, type: 1}], options = {contextInfo: {mentionedJid: [num, htod]}})
